...
@Repository
public class SessionTransportistaRepositoryImpl implements SessionTransportistaRepository {

	private final Logger logger = LoggerFactory.getLogger(SessionTransportistaRepositoryImpl.class);
	...
	@Override
	public void insertSession(SessionTransportista session) throws Exception {
	
		StringBuilder sql_insert_user = new StringBuilder();
		sql_insert_user.append("INSERT INTO tbl_session_transportista ");
	...
	@Override
	public SessionTransportista getSessionByToken(SessionTransportista session) throws Exception {
		logger.info("Repository.getSessionByToken init");
		List <SessionTransportista> lista = null;
		SessionTransportista bean = null;
		...
		sql_find_user.append("DATE_FORMAT(FEC_MODIFICA,'%d/%m/%Y %H:%i:%s') FEC_MOD, FOTO ");
  		sql_find_user.append("FROM tbl_session_transportista WHERE TOKEN = ? ");
		logger.info("Repository.getSessionByToken::"+session.getToken());
  		Object[] params = new Object[] {
			session.getToken()
		};
    	lista = (List <SessionTransportista> )jdbcTemplate.query(sql_find_user.toString(),params, new SessionTransportistaMapper());        	
    	if(lista!=null && lista.size()>0){
    		bean = new SessionTransportista();
    		bean = lista.get(0);
    	}
	...
	@Override
	public void deleteSession(SessionTransportista session) throws Exception {
	
		StringBuilder sql_insert_user = new StringBuilder();
		sql_insert_user.append("DELETE FROM tbl_session_transportista ");
